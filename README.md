...
- **Bounded Profile runtime.** Prevent duplicate launches of the same `user-data-dir`, supervise only official clients started by AgentDesk, close them on normal AgentDesk exit by default, and cap each Profile's `Crashpad/pending` at 100 files or 200 MiB. Five same-sized dumps within one minute fuse that Profile. Safe cleanup deletes only direct `.dmp` and `_sidecar.json` files; sessions, archives, configuration, SQLite, `codex-home`, and saved diagnostic samples remain untouched.
...
- **Profile 运行保护。** 同一 `user-data-dir` 不重复启动；AgentDesk 默认在正常退出时关闭自己启动的官方客户端，并把每个 Profile 的 `Crashpad/pending` 限制为 100 个文件或 200 MiB。一分钟内出现 5 个同尺寸 dump 会熔断该 Profile。安全清理只删除直属 `.dmp` 与 `_sidecar.json`，不会触碰会话、归档、配置、SQLite、`codex-home` 或留存诊断样本。
