...
test('额度总览随单账号额度一起刷新，单账号时隐藏，等级色点齐全', () => {
  assert.match(renderer, /function renderQuotaSummary\(\) \{[\s\S]{0,200}?renderQuotaOverview\(\);/);
  assert.match(renderer, /els\.quotaOverview\.hidden = profiles\.length < 2/);
  assert.match(renderer, /buildQuotaOverview\(profiles, state\.quotas/);
