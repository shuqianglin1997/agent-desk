...
test('渲染层只能选择内置适配器，Codex 使用槽位 CODEX_HOME 和受限沙箱', () => {
  ...
  service.send(profile, runtime.id, '检查项目');
...
test('运行环境归属账号，不能跨槽位写入或停止', () => {
  const child = fakeChild();
  const service = new RuntimeService({
    platform: 'linux',
    randomUUID: () => 'runtime-owned',
    shellSpec: () => ({ command: '/bin/sh', prefixArgs: [], extraEnv: {}, source: '测试' }),
    spawnImpl: () => child
  });
  const owner = { id: 'p1', appId: 'cursor', sessionRoot: '/profiles/p1' };
  const other = { id: 'p2', appId: 'cursor', sessionRoot: '/profiles/p2' };
  const runtime = service.start(owner, { adapterId: 'shell', cwd: '/workspace' });

  assert.throws(() => service.send(other, runtime.id, 'whoami'), /找不到这个内嵌运行环境/);
  assert.throws(() => service.stop(other, runtime.id), /找不到这个内嵌运行环境/);
});
...
test('Shell 输入先写入子进程，再广播 UI 回显，事件重入停止也不会空指针', () => {
  ...
  service = new RuntimeService({
    ...
    spawnImpl: () => child,
    onEvent: (event) => {
      if (event.stream === 'input') service.stop(profile, runtime.id);
    }
  });
  runtime = service.start(profile, { adapterId: 'shell', cwd: '/workspace' });

  assert.doesNotThrow(() => service.send(profile, runtime.id, 'echo ok'));
