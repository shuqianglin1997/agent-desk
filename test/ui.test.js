...
test('同账号识别双通道：手动 identityKey 与自动登录指纹都会点亮 ⛓', () => {
  const renderer = fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer.js'), 'utf8');
  const main = fs.readFileSync(path.join(__dirname, '..', 'src', 'main.js'), 'utf8');
  assert.match(renderer, /item\.identityKey === profile\.identityKey/);
  assert.match(renderer, /item\.identityFingerprint === profile\.identityFingerprint/);
  // 指纹在 main 侧运行时附加,不落盘
