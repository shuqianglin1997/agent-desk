...
test('内嵌控制台默认收起，账本行提供显隐开关；浮层自带 ✕/Esc 关闭出口', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'src', 'index.html'), 'utf8');
  const renderer = fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer.js'), 'utf8');
  ...
  // 浮层全屏覆盖会盖住小账本里的开关（唯一入口被自己遮住 = 关不掉）：
  // 必须有浮层内的 ✕ 收起按钮，且 Esc 优先关浮层
  assert.match(html, /id="runtimeCloseBtn"/);
  assert.match(renderer, /runtimeCloseBtn\?\.addEventListener\('click'[\s\S]{0,120}?consoleToggle\.click\(\)/);
  assert.match(renderer, /if \(state\.agentConsoleOn\) \{\s*\n\s*els\.consoleToggle\.click\(\);/);
});
