...
为了让这条逻辑始终落在实际工作上，全文沿用同一个例子：一个人手里有几个使用不同模型、额度和客户端的账号，现在要同时处理“远程会话仍显示旧数据”“不需要的 Agent 无法删除”和“Agent 卡片视觉异常”三类问题。最终目标不是让三个账号都保持忙碌，而是得到一个可以继续测试的集成版本：远端库存能够刷新，删除语义不会被旧数据复活，卡片在少量和大量 Agent 下都能稳定显示，相应回归与真实界面检查通过。

对多账号使用者来说，每一项工作都需要沿着同一条关系落到实际执行位置：

```mermaid
flowchart LR
  G["共同目标<br/>得到可测试的集成版本"] --> T["任务责任<br/>这次具体解决什么"]
  T --> A["Agent<br/>谁对这项结果负责"]
  A --> C["账号与模型<br/>用哪个登录、额度和能力"]
  C --> S["运行位置<br/>哪台设备、哪个客户端"]
  S --> H["会话<br/>当前掌握哪些上下文"]
  H --> W["Git 工作现场<br/>base、branch、worktree"]
  W --> D["交付与证据<br/>commit、diff、测试"]
```

*图 1：账号不是任务，模型也不是 Agent。一次可接续的派工，需要同时确定责任人、登录资源、运行位置、上下文和代码现场。*
...
*图 2：全文的管理闭环。图 1 说明一项工作怎样落到执行位置；本图说明这次结果怎样反过来改进下一次安排。*
...
*图 3：模型品牌只是输入之一。改变 effort、客户端、工具权限或上下文策略，都可能改变同一模型的工作表现。*

这也是为什么模型经验必须绑定版本和运行条件。网页里的同一模型、编码 CLI 里的同一模型，以及拥有浏览器或计算机控制工具的同一模型，不能直接当成一个稳定不变的成员。

多账号场景还需要再分清一层：Agent 表示相对稳定的工作角色，账号提供登录、额度和可访问的模型，运行位置把这个账号落到某台设备和某个客户端。一个“开发 Agent”可以先使用 Claude 账号阅读长代码库，额度或工作阶段变化后再由 GPT 账号接手集成；只要任务责任、Git 工作路径和交接证据保持连续，就不需要把它理解成一项全新的工作。反过来，同一个账号也可以在不同任务中承担不同责任，账号名称不应该自动变成岗位名称。
...
放回贯穿全文的例子，一次初始安排可能是下面这样。表里的模型只是说明匹配方法，不是固定推荐；如果手头只有两个账号，责任关系仍然成立，只是同一个账号需要串行承担几项工作。

| 工作包 | 当前责任 | 可优先尝试的配置 | 交付物 | 与其他工作的关系 |
|---|---|---|---|---|
| 远程会话旧数据调查 | 沿库存、缓存和刷新链寻找可复现根因 | 擅长长代码库和日志分析的高 reasoning 配置 | 数据来源图、根因证据、建议修改边界 | 可与删除调查同时开始 |
| Agent 删除调查 | 核对目录、Slot、tombstone 和旧库存防复活语义 | 约束保持较强的 GPT 或 Claude 配置 | 删除状态流、失败位置、回归条件 | 与会话调查共享部分数据语义 |
| 卡片视觉检查 | 在真实窗口中检查拉伸、重叠、滚动和信息层级 | Gemini、GPT 等多模态配置 | 截图证据、几何问题和验收尺寸 | 大部分可以独立推进 |
| 回归测试准备 | 把已经确认的语义转成可重复判定 | 成本较低、执行稳定的编码配置 | 测试用例和失败复现 | 等待共享语义收敛，但可提前准备框架 |
| 最终集成 | 选择改动、处理冲突并对共同版本负责 | 掌握主上下文且保留足够额度的主 Agent | 集成 commit、完整测试和真实界面结果 | 等待各工作包形成可审查交付 |

这张表只完成“派工”，还没有回答它们能否真正同时修改代码。下一步需要看任务依赖。
...
*图 4：调查可以并行，修改不一定可以。会话刷新和删除共享数据语义，应先收敛契约；卡片布局相对独立；所有结果最终在同一个集成版本上验收。*
...
*图 5：卡片修复可以从 B0 独立展开；会话刷新、删除和回归测试依赖 K1，应让提交父关系表达依赖；所有分支只有进入 I 后重新验证，才知道组合是否成立。*
...
例如，负责远程会话调查的 Claude 账号额度耗尽时，原责任不需要重新拆分。它可以在 `fix/remote-session-refresh` 上形成 checkpoint，记录已经确认的数据流和失败测试，再把 branch、head SHA、验证结果和未决问题交给另一个 GPT 或 Claude 账号。变化的是执行账号和会话，保持不变的是任务、工作路径和已经形成的工程事实。
...
*图 6：会话接续、Git 代码同步和本机工作树迁移是三件事。只有提交并推送的代码能通过 remote ref 被另一台设备稳定取得。*
...
同一台设备上切换账号时不一定需要经过 remote，但交接内容没有变化：新会话仍要知道任务、branch、head、dirty 状态和验证结果。账号切换解决额度与模型入口，不能替代工作状态交接。
...
1. 图 1 是否让多账号使用者先分清任务、Agent、账号、运行位置、会话和 worktree；
2. 同一个远程会话、Agent 删除和卡片问题示例，是否确实贯穿了派工、依赖、Git、验证和复盘；
3. 图 4、图 5、图 6 是否分别讲清任务依赖、Git 提交拓扑和跨设备交接；
4. 新增具体场景以后，原有模型与 Git 分析是否仍然保持完整，而没有退化成一份操作清单。
