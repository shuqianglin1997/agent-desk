...
>
> 本次增补：补充从提示、上下文到运行框架、工作环境与多 Agent 控制面的认知链，并校准行业证据、未来判断与 AgentDesk 当前定位。
>
...
这条发展路线可以从工程对象的逐层扩大看清楚。最内层是提示（prompt），也就是本次交给模型的目标、约束和输出要求。工作跨过一轮对话以后，结果还取决于上下文（context）：模型在当前时刻实际获得的提示、历史、资料、工具说明、工具返回和已有决定。提示回答“这一次要求什么”，上下文回答“模型现在依据什么来判断”。

一项工作需要经历调查、行动、观察和验证时，单次调用便展开为循环（loop）。循环规定模型如何根据新结果继续、重试、停止或交给人。工作中出现前后依赖、条件分支、并行节点和重新汇合时，可以用工作图（graph）表达整条路径。循环可以是工作图中的一段回路；工作图还可以同时容纳固定程序、模型调用、完整 Agent 运行和人工批准。

循环和工作图由运行框架（harness）实际托住。运行框架是组织模型、上下文、工具调用、状态保存、重试和验证的程序层。它在具体工作环境（environment）中执行；工作环境包括设备、客户端、项目目录、数据、依赖、账号、权限和预算。一次模型运行在这里取得输入、采取动作并留下结果。

多个 Agent 同时参与以后，还需要一层工作控制面。本文把分配责任、选择 Agent 与运行环境、应用配置、跟踪状态、处理交接并保存证据的平台能力称为工作控制面。它管理多名 Agent 之间的关系，运行框架负责一名 Agent 的具体执行，两者通过工作包、状态和证据相连。

提示、上下文、循环、工作图、运行框架、环境和控制面描述的是同一项工作在不同工程范围上的问题。它们互相嵌套，也会因产品架构而采用不同边界。当前使用者在分配任务、选择会话、反复测试、安排依赖、切换客户端、选择设备和交接结果时，已经在人工维护这些范围；平台的发展，是把这些关系逐步变成可保存、可验证和可持续运行的系统。
...
近年的工程实践已经沿着这条范围扩展。Anthropic 用“上下文工程”讨论怎样为每一步模型推理选择有限而高价值的信息；OpenAI 对 Codex agent loop 的拆解说明，运行框架负责连接用户、模型和工具；围绕长时间工作，OpenAI 与 Anthropic 都把状态、工具、检查、可恢复记录和环境约束放进运行框架。LangGraph 所称的“图工程”强调用节点、状态和边控制确定路径与 Agent 自主判断之间的边界；“循环工程”则把注意力放在调查、行动、验证、反馈和停止规则组成的可重复周期上。

这些名称反映了工程关注点的外扩，尚未形成一套统一职业等级。循环和工作图都可以由运行框架实现；运行框架依赖实际环境；上下文会在每轮循环中重新装配。2026 年出现的“环境工程”研究进一步把权限、产物、预算和人类介入视为可设计的工作条件。Anthropic 的 Managed Agents 也把持久会话、运行框架与沙箱环境拆成不同生命周期，说明长期 Agent 系统开始认真处理模型之外的状态和执行资源。
...
AgentDesk 在这条路线上的当前定位有两层。第一层是可用的个人开发协作工具：它把多个 Agent、平台账号、设备、客户端和会话放进同一工作视图，帮助使用者找到准确运行位置、保留会话连续性，并通过会话定位或任务包快照完成跨 Agent、跨设备和跨人的交接。第二层是一个可检验的原型：它用真实产品对象探索持续 Agent 身份、不同 Agent 配置、工作环境、会话来源、工作现场、证据和交接应该怎样被管理。
