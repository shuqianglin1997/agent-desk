...
>
> 本次增补：用连续性边界解释工程对象为何外扩，补充同一 TaskPackage 的便携与同 Mesh 两种交付通道，并校准代码事实、物理门禁与 AgentDesk 当前定位。
>
...
随着同一项工作跨越更长时间、更多行动和更多主体，工程需要保存的关系会逐步扩大。只完成一次回答时，提示（prompt）保存本次目标、约束和输出要求；工作进入下一轮以后，上下文（context）还要带回相关历史、资料、工具说明、工具返回和已有决定。工作开始调查、行动、观察和验证，循环（loop）就要保存每一步反馈以及继续、重试、停止或交给人的条件；出现前后依赖、条件分支、并行节点和重新汇合，工作图（graph）再把节点、状态和路径关系固定下来。

运行框架（harness）是让这些关系实际运转的程序层，它装配上下文，组织循环或工作图，调用工具，保存状态并执行检查。工作跨到具体设备、客户端、项目、数据和权限以后，工作环境（environment）又成为连续性的一部分：结果必须能够回到准确版本、依赖、账号、预算和可写范围。多个 Agent 共同参与时，工作还要跨过身份和责任边界；本文把分配责任、选择 Agent 与环境、跟踪状态、处理交接并保存证据的平台能力称为工作控制面。

这些名称描述的是同一项工作在不同连续性边界上的工程对象和观察视角。诸如“Loop Engineering”“Graph Engineering”以及相应的 Engineer 称呼，目前更多来自实践文章和产品生态，用来强调循环设计或工作图设计中的真实问题；它们还没有构成一套成熟、统一的职业头衔阶梯。具体系统也无需把所有名词做成独立层：工作跨过哪一种中断，就先保存足以让它在那道边界后继续的关系。

![同一项工作跨越更多中断时，必须保存的连续性边界怎样逐级扩大](./assets/continuity-boundary-expansion.svg)

*图：横向是同一项工作从单次请求走向多轮、多步、跨环境、跨 Agent 和持续业务责任；纵向是每经过一种中断后仍需保留的关系。后出现的工程范围继续携带前面已经形成的目标、上下文和执行事实，因此范围外扩来自连续性边界扩大。*
...
Skill 位于这条链的可复用方法层。持续身份让方法和结果拥有稳定归属，运行框架让方法能够反复执行，工作环境提供真实条件，工作状态保存尚未完成的责任，证据说明方法在哪些条件下有效。它们共同把一份 Skill 从可读说明变成某名 Agent 可以长期使用、校正和负责的工作能力。
...
![一项工作如何穿过 Agent 运行系统并把证据带回持续责任](./assets/agent-work-system.svg)

*图：一项工作从持续责任中形成当前工作包，经由确定的 Agent 身份与配置进入运行框架；运行框架装配上下文，按循环或工作图调用工具，并在实际环境中产生改变。产物和证据随后返回工作状态、Skill 与配置，也成为下游接受、重试、交接或升级的依据。*
...
前文介绍的工程对象，可以直接落到执行配置里。它们分别控制一次工作从要求进入模型，到结果回到业务状态的不同位置：

| 工程对象 | 本次需要决定的实际问题 | 在本文方法中的记录位置 |
|---|---|---|
| 提示 | 当前目标、边界、交付格式和停止条件怎样表达 | 工作包与本次运行配置 |
| 上下文 | 当前运行需要读到哪些历史、资料、决定、工具和证据 | 会话、上下文清单与本次运行配置 |
| 循环 | 行动后读取什么反馈，何时继续、重试、停止或升级 | 工作包状态、验证规则与运行记录 |
| 工作图 | 哪些节点前后依赖、可以并行、需要汇合或人工批准 | 临时工作图与共享契约 |
| 运行框架 | 哪个程序版本负责装配上下文、调用工具、保存状态和执行检查 | Agent 配置、客户端执行框架与运行记录 |
| 工作环境 | 在哪台设备、哪个项目与依赖中运行，具有什么数据、权限和预算 | 工程现场、资源清单与本次运行配置 |
| 工作控制面 | 谁负责哪一段工作，当前在哪里运行，怎样交接、治理和追溯 | Agent 身份、业务拓扑、工作包、状态与证据账本 |

三层配置把这些范围分开管理。工作规则或环节政策固定交付、风险和授权边界；Agent 配置保存长期 Skill、默认运行框架和稳定偏好；本次运行配置再选择具体提示、上下文、工作路径、账号、设备、客户端和环境。长期能力因此能够复用，本次条件也可以独立调整和比较。
...
快照生成以后有两种交付通道。便携通道保存标准 `.agentdesk-task` 文件，一次性解锁码通过另一条通信路径交给接收者，适合跨 Mesh 或人与人之间交换。同一 Personal Mesh（同一个使用者的可信设备网）内，双方同时支持直送协议且接收权限已经开启时，也可以传输同一份密文快照：文件按块发送，解锁信息进入目标绑定 envelope。这里的 envelope 是只允许目标设备在本次传输上下文中解开的加密小信封，它同时绑定来源、目标、传输编号和包哈希；接收端仍要逐次确认。接收记录中的来源设备 ID 与名称来自认证传输事实，来源 Agent 名与交接人是受任务包完整性保护的发送方声明，接收者据此理解交接说明，但不能把它们当成已经由对方 Agent 目录认证的身份。

两种通道只改变密文怎样抵达，不改变任务包内容和接受标准。接收方都要取得完整密文，验证加密认证、清单关系、整包与逐项哈希，再进入同一导入事务；错误解锁信息、目标不符、内容篡改或接收拒绝都在写入工作现场前停止。

![同一种不可变加密任务包如何通过两条通道汇入同一验证与导入](./assets/task-package-handoff.svg)

*图：人工检查点、会话、代码现场和明确附件先形成同一种不可变加密快照；便携文件与分开发送的解锁码、同 Mesh 密文分块与目标绑定 envelope 是两条交付通道，随后汇入同一套完整验证和导入。当前证据覆盖 Renderer、Preload、Main、PeerManager、TransferService、TaskPackageService 与 25/25 安全定向回归；真实窗口只覆盖直送资格和状态投影。真实 Electron WebRTC 数据面、物理双机与跨平台直送仍是开放门禁。*
...
这套结构适用于另一个 Agent、另一台设备或另一个人接续工作。接收者都要取得同一组责任事实、工程事实和验证边界；人与人之间可以选择双方认可的文件通道，同一可信设备网也可以在明确授权和逐次接受下走直送通道。任务包快照负责可验证地交付一次现场，成员身份、长期权限和后续协作关系仍由各自系统管理。
...
证据还连接业务环节的上下游。上游 Agent 的“已交付”需要下游 Agent 确认输入完整、格式可用和责任边界清楚；下游退回时，需要指出缺少的事实或未满足的条件。这样，交付与接受形成两端记录，结果责任能够沿业务拓扑追溯。
...
近年的工程实践已经沿着连续性边界外扩。Anthropic 用“上下文工程”讨论怎样为每一步模型推理选择有限而高价值的信息；OpenAI 对 Codex agent loop 和 App Server 的拆解，把用户、模型、工具、线程生命周期和持久记录放进同一个运行框架。围绕长时间工作，OpenAI 与 Anthropic 又把状态、检查点、可恢复记录、沙箱和环境约束纳入工程对象。LangGraph 所称的“图工程”强调用节点、状态和边安排确定路径与 Agent 自主判断；“循环工程”把注意力放在调查、行动、验证、反馈和停止规则组成的可重复周期上。

这些名称指向工程师正在维护的关系范围。循环可以位于工作图节点内部，工作图由运行框架执行，运行框架每轮装配上下文，再把动作交给真实环境。它们描述彼此嵌套的工程对象和观察视角；Graph Engineer、Loop Engineer、Environment Engineer 等称呼目前仍较松散，使用时需要先说明具体工程对象。2026 年的“Agentic Environment Engineering”研究把环境建模、生成、评价和应用整理成研究对象；Anthropic 的 Managed Agents 则把持久会话、运行框架与沙箱环境拆成不同生命周期。工程对象由一次提示逐步扩展到循环、工作图、运行框架、环境与工作控制面。

当系统同时运行多名 Agent，关注点继续向平台层扩大。OpenAI 的 Symphony 把项目管理事项映射到独立 Agent 工作区，并直接把任务看板称为 coding agents 的控制面；Google 的 A2A 协议为不同厂商 Agent 提供能力发现、任务状态、消息与产物交换；Microsoft Agent 365 则把 Agent 注册、身份、访问控制、可观测性和治理放进统一控制面。这些实践支持一个正在形成的方向：Agent 工作需要长期身份、受控环境、持续状态、跨 Agent 协作和可追溯治理。Agent 最终怎样进入具体业务环节，仍需由真实业务运行、责任边界和长期证据继续验证。

本文把相关判断分成三层：

| 判断层 | 当前可以成立的内容 | 仍需继续验证的内容 |
|---|---|---|
| 已发生的使用事实 | 多个 Agent 已通过不同账号、会话、工具和设备参与工作；上下文、运行框架和环境会显著改变结果 | 怎样用统一数据长期比较不同组合 |
| 正在形成的工程层 | 持久会话、隔离环境、Agent 编排、协议互通、身份与治理正在进入真实产品和研究 | 各平台最终采用哪些共同对象与接口 |
| 面向未来的组织假设 | 稳定 Agent 能力可以逐步承接持续业务状态，并发展为不同业务环节的负责人 | 哪些环节适合长期授权，怎样评价责任、成本和风险 |

AgentDesk 在这条路线上的当前定位有两层。它首先是面向今天协作损耗的个人开发协作工具：把多个 Agent、平台账号、设备、客户端和会话放进同一工作视图，帮助使用者找到准确运行位置、保留会话连续性，并通过会话定位或任务包快照完成跨 Agent、跨设备和跨人的交接。它同时也是一个可检验的原型，用真实产品对象探索持续 Agent 身份、不同 Agent 配置、工作环境、会话来源、工作现场、证据和交接应该怎样被管理。

便携 TaskPackage 已经提供跨边界交接的固定快照；同 Mesh 直送当前按 Preview 边界推进，Renderer、Preload、Main、PeerManager、TransferService 与 TaskPackageService 的代码路径已经接通，25/25 安全定向回归覆盖目标设备独占 envelope、逐次接受、撤权、过期、防重放、清理与同一密文快照的便携回退。真实窗口 21/21 只覆盖直送资格和状态投影，现有隔离双 endpoint E2E 也尚未发送 TaskPackage；真实 Electron WebRTC 数据面、物理双机、公网网络、长连接恢复和跨平台权限门禁仍需分别关闭。这种证据分层也限定了原型的表述：已经能用的部分解决当前问题，尚在验证的部分用来检验未来控制面应当保存哪些关系。

大型模型与办公平台已经开始建设原生 Agent 运行、编排、身份和治理能力，未来很可能吸收其中相当一部分通用基础设施，最终界面与对象也可能采用不同形式。AgentDesk 当前解决的协作损耗独立成立；它的研究价值在于把未来判断落实成今天能够使用、观察和修正的系统，并持续用真实结果校正未来形态。
...
这条路线同时包含当下事实、正在形成的工程能力和面向未来的组织假设。当前协作可以立即用共同交付物、工作包、配置、工程现场、交接与证据得到改善；未来形态则由后续真实运行继续校正。平台的价值从减少今天的协调损耗开始，也通过这些实际记录逐步回答 Agent 能否稳定承担更长责任。
...
- Xin et al., [EurekAgent: Agent Environment Engineering is All You Need For Autonomous Scientific Discovery](https://arxiv.org/abs/2606.13662)
...
- Microsoft, [Microsoft Agent 365 overview](https://learn.microsoft.com/en-us/microsoft-agent-365/overview)
