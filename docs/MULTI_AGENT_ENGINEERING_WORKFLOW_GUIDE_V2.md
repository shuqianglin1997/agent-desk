...
本文先分析多个 Agent 怎样在当下的工具和会话中建设 Skill，随后说明稳定能力怎样发展为长期业务责任。接下来的各章依次回答：一次共同工作如何确定终点，如何拆分并安排先后，如何为不同 Agent 选择能力和运行条件，如何保存别人可以接着使用的工作记录，以及如何用证据决定哪些结果和方法值得继续采用。
