...
左上角 `↻` 会查询固定仓库 `shuqianglin1997/agent-desk` 的最新正式 GitHub Release，并按语义版本比较当前版本。
...
`profiles.json` 使用临时文件写入并保留 `profiles.json.bak`。主文件损坏时优先从备份恢复，避免重启后账号槽位被静默重置。
...
9. 从可写目录运行旧 portable 版，点击 `↻` 能下载、校验、替换并重启到新版本。
