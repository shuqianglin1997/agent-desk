...
src/
  ...
  yard/
    ...
    scene.js           画布引擎：8fps tick、角色移动、命中检测、昼夜、fx
    yard.css           庭院布局与像素皮肤（scoped 到 body[data-view=yard]）
```

- **渲染层与业务解耦**：`scene.js` 只吃 renderer 喂的数据（`update`），不碰 IPC、不碰业务状态。
- **昼夜跟随主题**：`◐` 深色 = 夜晚（暖窗 + 台灯 + 萤火虫）。深夜（23:00〜05:00 且夜晚配色）没在干活的猫都睡下。
- **性能**：8fps；窗口失焦 / 隐藏时暂停心跳；`prefers-reduced-motion` 时全静帧、猫瞬移到位。
