...
正式美术资产的完整清单、锚点、动画、区域语义和交付顺序见 [YARD_ART_ASSETS.md](YARD_ART_ASSETS.md)。当前程序化猫与庭院仅是功能占位，不作为画师必须复刻的风格稿。
...
庭院中的设施是真实操作入口，不是静态背景。拖拽时才显示语义命中层；放下只生成意图，再由 renderer 确认和执行，画布动画本身没有启动 App 或运行命令的权限。

| 区域 | 真实行为 | 安全边界 |
|---|---|---|
| 工作亭 | 打开未运行的官方 App；已运行时聚焦账号 | 打开前确认；失败显示真实错误气泡 |
| 邮筒 | 复制当前会话交接信息 | 必须有当前会话并确认，不复制完整聊天 |
| 任务道 | 把当前会话加入 Agent 待办 | 必须有可用 Agent CLI；最多 20 条；首次开启仍需系统确认 |
| 池塘 | 滚动到当前会话详情 | 无会话时明确禁用 |
| 瞭望点 | 定位到内嵌终端 / Agent 工作台 | 不自动执行；只打开工作台 |
| 树下草坪 | 保存猫的自定义位置 | 坐标限界并写入稳定设置 |
...
src/
  ...
  yard/
    ...
    atmosphere.js      系统时段 + 可复现的 20–45 分钟自动天气
    interactions.js    场景语义区域、命中和安全意图
...
- **时间**：庭院右上角控件 `跟随 / 白天 / 黄昏 / 夜晚`。「跟随」按本机时钟自动映射白天、黄昏、夜晚，不再跟随深浅主题；也可手动锁定。真实深夜且处于夜晚时段时，未工作的猫会睡下。时段决定地形离屏缓存的 key。
- **天气**：控件 `自动 / 晴 / 多云 / 雨 / 雪`。「自动」是本地、可复现的环境节奏，每 20–45 分钟变化一次，切回前台会立即按当前时间恢复；它不宣称是真实气象。手动天气保持不变，直到用户切回自动。时间与天气选择存进 `settings.json`。
...
- 像素资产目前为程序化绘制，零外部图片。正式替换必须遵循 [YARD_ART_ASSETS.md](YARD_ART_ASSETS.md) 的锚点和语义合同；若美术发现逻辑不合理，应同步修改区域/状态逻辑，而不是强行迁就旧构图。
