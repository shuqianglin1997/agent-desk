...
当前开发版同时管理本机账号槽位、逻辑会话、本机工具和一个人的可信设备网。Personal Agent Mesh 已贯通有人值守跨设备代码链路，但仍不替用户组织接下来的对话，也不把历史会话变成任务；物理双机、真实公网和跨平台权限矩阵仍属于发布验收。

## 2. 功能地图

| 区域 | 用户入口 | 实际作用 | 当前决定 |
|---|---|---|---|
| 全局栏 | Device Lens、设备入口、更多 | 选择全部设备/某台设备并进入设备中心；排行、更新、工具、帮助、语言和主题收进低频菜单 | 已收敛；Presenter 切换留在账号呈现层，七行骨架不变 |
| Agent 与运行位置 | 打开账号、新增、运行位置选择、管理 | 新建 Agent/账号绑定/本机 Slot，并把打开、路径、诊断、重扫等动作落到确切 Slot | 已实现；全局 Agent 与本机 Slot 分层，均可删到零 |
...
| 会话操作 | 复制会话信息、打开所在位置、导出 Markdown | 复制路径和坐标、在文件管理器定位、导出受支持的单条 transcript | 收紧；复制只有一种格式 |
| 会话身份 | Codex 根 thread、压缩检查点、guardian/subagent rollout | 一条用户逻辑会话对应一行，物理文件与内部执行单独分类 | 第一批已实现；父缺失的内部记录也不提升为用户会话 |
| 状态 | 关注提示、活动状态、额度 Beta、全院额度 | 提醒路径异常、显示本地活动和官方可得额度 | 保留；活动与额度互不冒充 |
| 猫猫庭院 | 猫、天气、时间、拖放、提醒、小账本、排行 | 同一账号/会话数据的可视化 | 保留；拖放只打开账号、聚焦会话或保存位置 |
...
| 应用更新 | 顶栏“更新” | 检查可信 Release，支持的平台校验后替换 | 保留；正式 macOS 包仍需签名和公证 |
| Personal Mesh 身份与设备 | 顶栏“设备”、添加设备、权限、撤销 | 建立系统保护身份、一次性加密配对、设备权限与可删到零的成员目录 | 代码已实现；LAN 临时入口、签名成员事件和撤销防复活均有自动化 |
| 全局 Agent 与跨设备库存 | 设备 Lens、Agent/会话列表 | 同账号跨设备去重、同设备多账号分离、强会话副本折叠、离线快照 | 代码已实现；设备事实来源单写，弱标识不猜测合并 |
...
| 远程查看与控制 | 设备详情“查看”、主窗口第 6 行隔离 Remote Surface | 目标端逐次同意后查看屏幕、控制键鼠、切换显示器 | 代码已实现；返回释放输入并保留后台查看提示，断开才结束媒体 |
| 多设备控制台 | 第 6 行 Remote Surface 单屏/网格 | 最多四路、一个活动画质、唯一输入目标和公开网络统计 | 代码已实现；切换/断线/撤销均释放按键 |
| 公网会合与诊断 | 设备“网络设置”“连接诊断” | HTTPS 信令、STUN、短期 TURN、LAN/直连/中继状态 | 代码已实现；服务端可自托管，不接收业务内容；真实 NAT/coturn 待物理验收 |
| UI 上下文 | Device Lens、Agent、Slot、focus/checked、副本、设备详情、远控、传输草稿 | 保持每种对象和动作目标独立，并提供原子导航 | 已实现；`selectedProfileId` 不再是业务状态，render/filter 无选择副作用 |
| 真实窗口验收 | `npm run accept:ui` | 在临时 userData 的 1040 × 840 Electron 窗口执行 13 条任务路径 | 已通过；覆盖三语、主题、双 Presenter、对象管理、多副本、远控和 reduced-motion |
...
- 默认账号、独立账号和最后一个账号使用同一删除规则。
- Codex 使用用户根 `session_id` 作为列表身份；压缩不新增行，guardian/subagent 默认隐藏。
- Device、AgentIdentity、AccountBinding、AgentSlot 和远端库存已进入独立 Mesh 存储；同账号跨设备/形态不重复、同机多账号不误合并。
...
- 远控媒体独立于普通 Main Renderer，但视觉上只替换主窗口第 6 行；目标端持续可见并可立即停止，多设备模式始终只有一个输入目标。
- 公网信令只转发固定配对和 offer/answer，TURN 凭据短期存在内存，诊断不显示 IP、SDP 或凭据。
- Agent、Device Lens、Slot、Conversation 与 SessionReplica 的 UI 上下文已经拆开；设备详情导航、搜索、刷新和渲染不会再用旧 Profile 状态静默改写动作目标。
...
- 两台物理电脑、真实家庭 NAT/CGNAT、coturn UDP/TCP/TLS、macOS/Windows 四向权限、DPI 和 IME 尚未完成验收；这属于发布门禁，不是用更多模拟测试可以替代的功能描述。
