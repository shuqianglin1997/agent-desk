...
| 固定页面骨架 | 顶部 Agent、左下会话、右下详情、Footer | 庭院/卡片与 Agent/Slot 操作归顶部，会话浏览归左下；右下只承载会话、额度、远控，Footer 只保留全局状态 | 已实现；主区始终恰好三个面板，不插入额外信息轨 |
...
| 状态 | 活动弹窗、额度详情、Footer 全局状态 | 提醒路径异常、显示本地活动和官方可得额度；Footer 显示今日账本与提醒总开关 | 保留；活动是临时弹窗，额度才使用右下详情，二者都不占独立页面行 |
| 猫猫庭院 | 顶部 Agent 面板内的庭院/卡片切换、排行 | 同一账号/会话数据的可视化 | 保留；两种模式共用选择，拖放只打开账号、聚焦会话或保存位置 |
...
| 真实窗口验收 | `npm run accept:ui` | 在临时 userData 的 1040 × 840 Electron 窗口执行 14 条任务路径 | 已通过；覆盖固定三面板、四个 Header 入口、三语、主题、庭院/卡片、对象管理、多副本、远控和 reduced-motion |
