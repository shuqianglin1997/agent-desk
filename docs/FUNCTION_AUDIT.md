...
| 远程查看与控制 | 设备卡“查看”、独立 Remote Console | 目标端逐次同意后查看屏幕、控制键鼠、切换显示器 | 代码已实现；持久权限和本次 consent 缺一不可，无无人值守 |
| 多设备控制台 | Remote Console 单屏/网格 | 最多四路、一个活动画质、唯一输入目标和公开网络统计 | 代码已实现；切换/断线/撤销均释放按键 |
...
- 远控独立于主窗口，目标端持续可见并可立即停止，多设备模式始终只有一个输入目标。
