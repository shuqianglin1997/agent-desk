...
| 全局 Agent 与跨设备库存 | 设备 Lens、Agent/会话列表、远端“重扫” | 同账号跨设备去重、同设备多账号分离、强会话副本折叠、离线快照 | 代码已实现；首份库存落库后才完成连接，已认证重连与固定 `remoteInventory:refresh` 可立即取新快照，连接存续时每 4 分钟发布有界全快照；库存权限实时复核且撤权断链，刷新请求单飞、节流并限制等待队列；revision 增量仍待后续演进 |
...
- Device、AgentIdentity、AccountBinding、AgentSlot 和远端库存已进入独立 Mesh 存储；同账号跨设备/形态不重复、同机多账号不误合并；首库存落库屏障、显式远端刷新和 4 分钟全快照恢复基线已经补齐。
