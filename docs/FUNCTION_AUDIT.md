...
| 会话列表 | 当前 Agent/全部 Agent、显示、搜索、表头排序 | 与 Device Lens 正交地只读浏览各客户端历史 | 保留；Mesh 聚合失败时健康本地 Profile 继续只读扫描，明示本地模式并暂停跨设备动作；不产生任务或运行状态 |
...
| 本地会话容灾 | Mesh 密钥/远端库存失效隔离 | 本地 Profile 只读扫描继续返回最新会话，标明本地模式并暂停跨设备动作；精确 ad-hoc macOS 包普通启动延后 Keychain，点击“设备”才解锁 | 代码纵向已实现；新安装包在不使用 mock Keychain 的普通启动与再次重启中都恢复同一 Agent/Slot、展示 22 条本地会话并停用发送 |
...
- ProvisioningJob 的普通启动恢复、密钥解锁恢复与轮询均为 observation-only：无身份保持 waiting-login，已有身份可提交 ready，但不调用安装入口或客户端启动；用户显式操作与后台 in-flight 重叠时会在观察完成后继续执行，不被后台结果吞掉。
