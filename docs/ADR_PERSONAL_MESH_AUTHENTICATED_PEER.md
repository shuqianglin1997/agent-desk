...
2026-08-13 的首次物理双 Mac 局域网验证进一步证明了 host/UDP DataChannel、设备证书认证和独立 catalog 落库。随后真实大库存的首个 192 KiB 原始块在 Base64 与签名封装后超过该链路的单消息边界并触发 `datachannel-error`。实现把发送上限收紧到 96 KiB，并增加完整签名信封小于 192 KiB 的回归。两端升级到 0.9.4 后，562,009 字节真实库存中的 9 个 Slot 与 638 条 SessionReplica 已完整落库；显式刷新和 4 分钟周期全量同步分别把 revision 从 7 推进到 8 和 9，认证连接持续 5 分钟无错误或断开。

## 尚未由本 ADR 证明

- 不同 NAT、CGNAT、公网切换和 TURN 强制回退；
- TURN 回退与短期凭据服务；
