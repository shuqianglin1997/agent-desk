...
主窗口第 6 行的 Remote Surface 最多同时保存四个远程查看会话，并提供两种属于同一工作区状态的布局：
...
7. 布局与媒体诊断只存在于第 6 行隔离 Remote Surface，不改变 AgentDesk 主窗口七行骨架或普通 Main Renderer 的安全边界。
...
Remote Surface 固定落在 1040 × 840 主窗口的第 6 行；当前本机内容区实测边界为 1040 × 347。紧凑高度仍保留四格、当前设备、显示器、画质、暂停、控制、断开和仅查看/控制状态；显示器名称按可用宽度缩短，完整名称与尺寸保留在选项提示中。
...
- 既有四台模拟设备曾对 Remote Console 页面完成 1180×760 与 780×520 布局验收；1.7 迁移后又在真实 1040×840 AgentDesk 窗口中验证第 6 行 1040×347 内嵌边界，未读取真实桌面、未请求系统权限、未发送系统输入。
