...
一个 Remote Console 最多同时保存四个远程查看会话，并提供两种属于同一窗口状态的布局：
...
7. 布局与媒体诊断只存在于独立远控窗口，不改变 AgentDesk 主窗口七行骨架。
...
窗口默认 1180 × 760，可缩放到既有最小 780 × 520。窄窗口保留四格、当前设备、显示器、画质、暂停、控制、断开和仅查看/控制状态；显示器名称在窄窗口缩短，完整名称与尺寸仍保留在选项提示中。
...
- 使用四台模拟设备对真实 Remote Console 页面做了 1180×760 与 780×520 窗口截图验收，未读取真实桌面、未请求系统权限、未发送系统输入。
