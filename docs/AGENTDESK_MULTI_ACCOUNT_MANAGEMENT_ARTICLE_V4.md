...
本文采用 AgentDesk 全部规划功能完成后的产品形态。内容分为工作方式、操作说明和技术方案三部分，面向需要管理自己多个 AI 账号和可信设备的使用者，也供后续产品与开发验收使用。
...
主窗口保留七行布局。设备加入以后，只增加设备范围、运行位置和跨设备动作，不重新排列原有基本结构。

![图 5：主窗口七行布局、运行位置和独立远程控制窗口](./assets/agentdesk-multi-account-v4/05-main-interface-map.svg)

*图 5：顶栏设备范围和会话范围互不绑定；Agent 全局只显示一次；所有副作用动作都落到明确运行位置；远程控制使用独立窗口。*
...
目标项目缺少未提交文件、素材或临时输出时，点击“发送文件”，从来源电脑选择需要的文件或文件夹。目标端显示保存位置和相对路径。

同名文件默认不覆盖。每项冲突可以选择跳过、保留两份或覆盖。文件按块校验，断线后从已经确认的位置继续。目录穿越、危险符号链接和目标磁盘空间不足都会在写入前停止。
...
远程窗口始终提供“暂停输入”“断开控制”和“全部停止”。目标电脑持续显示来源设备名称、当前是查看还是控制，并提供本机停止按钮。
...
| 独立远程控制窗口 | 承载画面和输入，不把高风险能力放进主窗口前端 |
