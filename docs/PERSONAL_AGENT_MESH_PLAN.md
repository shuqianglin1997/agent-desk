...
>
> 完整产品审阅稿：`docs/PERSONAL_AGENT_MESH_OWNER_REVIEW.html`。它按真实场景、功能变化、完成态界面、异常处理和验收顺序供所有者审阅；`docs/PERSONAL_AGENT_MESH_REVIEW.html` 保留为会话身份问题的专项技术图解。本文仍是实施时必须完整重读的单一基准。
...
7. 现有主窗口的大骨架、庭院/经典双视图和账号工作流不能被整体推翻。
...
当前主窗口固定为 1040 × 840，app-shell 是单列七行：
...
| 现有区域 | Mesh 后职责 | 布局约束 |
|---|---|---|
| 顶栏 | 增加“全部设备/某台设备”视角、设备入口和在线数量 | 不增加侧栏，不改变顶栏所在行 |
| Presenter | 展示去重后的全局 Agent；选择设备时只做筛选 | 一只猫始终是一个 AgentIdentity，设备不变成猫 |
| 账号控制条 | 展示所选 Agent，并选择具体设备/客户端运行位置 | 原有按钮位置和主次关系保留 |
| 需要留意 | 权限缺失、设备离线、传输失败、版本不兼容 | 继续是可收起提醒区 |
| 全院额度 | 按 AccountBinding 去重后的额度摘要 | 同一登录多设备快照不能相加，必须标记来源和时间 |
| 会话表 + 详情 | “当前 Agent/全部 Agent”叠加顶栏设备视角 | 表格仍是主工作面，不新增第三个混合范围 |
| 状态栏 | 显示连接路径、同步、传输和错误摘要 | 不承担主要控制 |

2026-08-11 所有者完成主工作台 UI 规划稿审阅后，进一步批准以下呈现基线。它只重组七行骨架内部的信息层级，不改变 AgentIdentity、Device Lens、AgentSlot、SessionReplica、SessionPointer 或 IPC 语义：

1. Presenter 默认采用紧凑 Agent 名册；庭院继续保留为用户主动切换的完整模式，并与名册共用同一选择和业务状态。
2. 顶栏只常驻设备 Lens、设备入口和“更多”；排行、更新、工具、帮助、语言与主题进入低频操作菜单。庭院/名册切换放回 Presenter 自身，不再占用全局命令区。
3. 账号控制条只常驻“打开账号”和“新增”；路径、诊断、重扫、编辑、移除和位置统一进入一个上下文菜单，确切运行位置选择器与额度摘要继续可见。
4. 会话工具栏常驻 Agent 范围、搜索和“显示”菜单；精简/详细收进“显示”。只有选中会话后才出现上下文选择条，其中“复制会话信息”仍是唯一填充主按钮，“发送到设备”是相邻次级动作。
5. 会话详情优先展示 Agent、运行位置、项目和最后活跃时间；来源、创建时间与稳定坐标收进默认折叠的“技术信息”。“打开当前位置”保持可见，导出进入详情“更多”。
6. 设备中心改成“设备列表 + 所选设备详情”；列表只承担选择，连接、远控、传输、权限、诊断、重命名和撤销在详情中按主次分组。
7. 设备中心不再重复展示一份可独立浏览的全局 Agent 目录；详情只展示“所选设备上的 Agent”，并继续链接同一全局 AgentIdentity。
8. 内嵌 Remote Surface 移除重复的 AgentDesk 品牌抬头和独立返回层；设备标签、连接状态、画面与控制保留在紧凑工作区工具条中，返回能力以紧凑动作存在，不复制主窗口导航。

### 11.2 顶栏细节建议

- 在全局操作区增加“设备”按钮，建议放在“工具”之前。
- “设备”按钮同时提供快速视角：“全部设备”以及每一台具体设备；默认是“全部设备”。
- 顶部上下文显示“全部设备/设备名 · 庭院/经典 · 数据新鲜度”。
- 设备在线数量用小徽章表达，不常驻展示 CPU/GPU 细节。
- 选择设备只是筛选 Agent 和会话，不会创建另一套账号状态，也不会改变 Agent 身份。
- 完整设备管理进入设备中心。
- 不把设备列表做成永久侧栏。
- 常驻操作只保留设备 Lens、设备入口和“更多”；排行、更新、工具、帮助、语言、主题归入“更多”。
- 庭院/紧凑名册切换属于 Presenter 的显示方式，不再与设备等全局操作平铺。
...
精简/详细进入一个“显示”菜单，不再作为两个常驻分段按钮平铺。未选择会话时不展示复制与发送动作；单选或多选后出现上下文选择条，“复制会话信息”为唯一填充主按钮，“发送到设备”为紧邻的次级动作，并提供明确的取消选择。这样不降低复制按钮尺寸和权重，也不挤占常驻搜索空间。
...
点击某台设备后在同一详情面板查看“此设备上的 Agent”，但这些条目仍链接回全局 AgentIdentity，不在设备中心再展示一份重复的全局 Agent 目录，也不创建可独立重命名的账号对象。
...
- workspaceMode：sessions、devices、pairing、remote；
...
| E. 交互契约 | 保留七行骨架；设备 Lens 与 Agent 范围正交；“复制会话信息”仍是唯一主按钮且只含路径/坐标 | 主窗口改动范围和日常工作效率 | 若改变主骨架或复制格式，必须先单独审阅 UI，不与 Mesh 实现捆绑 |
