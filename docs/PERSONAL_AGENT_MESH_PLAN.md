...
6. 当前来源优先级为：用户已选位置、本机副本、在线副本、最新离线副本；若仍有歧义，在执行动作前要求选择，不随机猜测。
...
新增“发送到设备”应是次级动作。具体候选位置：

1. 紧邻“复制会话信息”的次级按钮；
2. 选中后出现的小型操作菜单；
3. 详情区底部的次级动作。

默认建议采用候选 1，但必须先做 1040 × 840 真机布局验证；若工具栏拥挤，优先收缩搜索框，不降低复制按钮尺寸和权重。
...
- 直连能力和权限状态；
- 最近在线；
- 操作：选择、控制、权限、诊断、重命名、撤销删除。

空状态应直接提供“添加设备”，不自动制造默认远端设备。

设备卡片允许按“可用资源”“活跃 Agent”“最近使用”排序，但不出现“自动分配任务”按钮。

点击某台设备后可以查看“此设备上的 Agent”，但这些条目仍链接回全局 AgentIdentity，不在设备中心再创建一套可独立重命名的账号对象。
