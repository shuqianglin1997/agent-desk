...
23. 全局 Agent 目录没有“哪台设备说了算”的固定主机。权威输入是所有通过成员证书、设备签名、来源序列和因果关系验证的目录事件；各设备对同一组事件采用同一套确定性规则得到同一目录。
24. 目录事件只决定 Agent、AccountBinding、Blueprint 及删除墓碑；每台设备仍只对自己的 Deployment、Slot、会话副本和本地路径拥有事实写权限，另一台设备不能用目录同步覆盖这些本机事实。
25. 显式删除 Agent 后，同一 agentId 永久受 tombstone 约束。旧库存、旧快照、离线改名和旧运行位置都不能恢复它；用户以后主动创建同名 Agent 时必须获得新的 agentId。
26. 一次整项工作需要由另一个 Agent、另一台设备或另一个人接手时，使用用户显式生成的 TaskPackage：它固定人工填写的阶段检查点、会话内容、Git 基线与已跟踪差异和明确附件；来源副本始终保留，接收方先验证再导入。TaskPackage 不改变全局 Agent 目录权威，不自动同步或合并两端，也不把 Personal Mesh 扩展为团队成员系统。
...
34. ProvisioningJob 的普通启动恢复、密钥解锁后恢复和定时轮询只允许检查安装状态、准备受管目录、观察安全身份指纹并提交已经成立的就绪事实；不得打开下载页、登录页或官方客户端，也不得把后台 in-flight 检查当作用户点击。只有用户明确点击“首次准备并打开 / 继续安装 / 继续登录 / 打开账号”，或远端有人值守确认完成后，才能产生对应外部界面。已有身份信息的确切 Profile 可在后台收敛为 ready，但仍不自动拉起客户端。
...
- 跨设备项目路径映射；
- 用户选择的文件传输；
- 用户显式生成和导入的加密 TaskPackage，用于 Agent、设备或人与人之间接续一项完整工作；
- 固定语义的远端动作，例如打开已知账号、聚焦已知会话；
- 屏幕查看、鼠标键盘控制和多显示器选择；
- 多设备控制台、连接诊断、断线恢复和审计；
- P2P 直连、必要时加密中继；
- macOS 与 Windows 的权限适配。
- AgentDesk 所启动官方客户端的 Profile 进程归属、重复启动阻断、正常退出收口、Crashpad 容量上限、崩溃风暴熔断与窄范围安全清理。
...
### 7.10 TaskPackage：可交付的工作检查点

TaskPackage 表示用户在一个明确时刻固定下来的整项工作快照。它用于把工作交给另一个 Agent、另一台设备或另一个人继续，不是持续更新的任务对象，也不等于第 12 节的 SessionPointer。

TaskPackage 至少包含：

- packageId、创建时间和可选父包来源；
- 来源 Agent、客户端、设备和用户填写的交接人；
- 人工填写的目标、已完成、下一步、阻塞/风险与验收标准；
- 一份会话内容：适配器支持时为可验证的原生会话根记录及其内部子记录，否则为只读 transcript；
- 可选项目检查点：Git remote、分支、当前提交、工作树状态和已跟踪文件的二进制 patch；
- 用户通过系统选择器明确加入的附件；未跟踪文件只列在工作树状态中，不自动读取正文；
- 每项内容的逻辑路径、长度和 SHA-256，以及整包版本、大小和加密参数。

硬规则：

1. TaskPackage 是不可变快照。导出后来源会话、项目和文件继续存在；产品不提供“导入成功即自动删除来源”的伪移动。
2. 安全移动顺序固定为“复制 → 解密与逐项校验 → 导入/保存 → 用户按需归档来源”，失败不触碰来源。
3. TaskPackage 只保存用户填写的检查点，不读取会话后自动编写摘要、下一步或验收标准。
4. 原生会话只能由明确版本的客户端适配器导入；当前只支持 Codex rollout JSONL。Claude CLI、Kimi Code、Kimi Work 等已有导出能力的来源使用只读 transcript，不能伪装为客户端原生历史。
5. Codex 原生导入同时携带根会话和归属于它的 guardian/subagent 内部记录，目标端重新验证记录身份，不覆盖同 ID 的不同内容；成功后标题标注 `原标题 · 来自「交接人 / 来源 Agent」`。
6. 项目代码继续以 Git remote/branch/head 为基线；已跟踪未提交差异以 patch 携带。完整仓库、未跟踪文件正文、依赖、凭据和本机环境都不自动打包。
7. 便携 `.agentdesk-task` 文件使用流式 AES-256-GCM 和 scrypt 派生的一次性高熵解锁码。外层不暴露标题、Agent、项目或文件名；解锁码不写入包、历史或日志，必须与文件分开发送。
8. 解密暂存只在 Main 控制的私有临时目录存在，取消、成功、30 分钟草稿过期或应用退出时清理；本地历史只保存包 ID、方向、模式、展示信息和本机位置，不保存解锁码。
9. 接收方先选择包文件并验证，再选择本机 Agent 与确切 Profile。原生会话和任务资料先作为一个可回滚事务提交；提交成功后的历史记账或打开客户端失败不得反向删除已导入内容。
10. TaskPackage 不参加 Agent catalog、Device inventory 或 ConversationReplica 的自动同步。跨同一 Personal Mesh 的直接传送可在后续复用受认证传输通道；跨人与跨 Mesh 的当前基线是便携加密文件，不能把文件交换写成已经建立团队信任。

工程方法中的 WorkPackage 表示一项仍在推进的责任节点；产品中的 TaskPackage 表示该责任在某个时刻可被别人取得和验证的固定快照。两者可以关联，但生命周期和权威不同。
...
- transferId；
- type：session-pointer、file、task-package、supported-session-bundle；
- sourceDeviceId；
- targetDeviceId；
- state；
- manifest；
...
| task.package.receive | 接收同一 Mesh 可信设备直接发送的 TaskPackage；每次仍须检查来源并明确接受 | 关闭，用户开启且逐次确认 |
...
普通首次使用从主工作台的“创建第一个 Agent”开始，不要求用户先理解 Personal Mesh 或进入设备中心；已经在管理多设备的用户仍可从设备中心显式创建。两个入口必须收敛到同一套可恢复事务：

1. 用户填写第一个 Agent 的名称并选择受支持的客户端类型；若已发现本地 Profile，先展示无损迁移预览，由用户确认哪些既有登录进入初始员工库。
2. 用户确认后，Main 才建立本机 Agent 目录、本机设备身份和恢复信息；同一动作重复触发必须返回既有结果，失败不得留下半份目录或设备身份。
3. 本机自动成为第一台设备，但此时不开放局域网端口、不发布公网在线租约、不建立远端连接，也不授予任何远端能力。
4. 当前 Profile 逐个映射为本机 AgentSlot，不移动文件；现有本地 Identity Group 迁移为初始 AgentIdentity，无法确认的槽位保持独立，宁可多一个也不误合并。
5. 当前 Session 继续属于原 Slot，同时进入全局 Agent 读取视图。
6. 新创建的 Agent 随后进入本机“首次准备并打开”；软件安装、官方登录、验证码和系统权限继续按第 10.10 节明确等待，不被首次向导绕过。

设备中心的“创建个人设备网”是同一事务的高级入口，不得形成另一套目录、密钥或迁移逻辑。首次向导完成版本必须单独记录；只有该版本的向导实际展示完成后才能写入完成状态，旧版本升级时按版本规则重新展示新增的必要步骤。

### 10.2 添加设备

“添加设备”使用一个围绕任务进度组织的向导，明确区分“建立信任、建立连接、取得员工库、取得设备库存”四个结果，不能把配对成功写成全部连接已经完成：

1. 已加入的设备创建一次性邀请；新设备输入短码或扫描二维码。二维码、短码和必要的复制动作都留在当前步骤，不要求用户另找提示词。
2. 两端展示设备名称、系统和安全指纹，并分别确认对方身份；任一端未确认时都不得签发成员证书。
3. 新设备获得有范围的成员证书后，向导只标记“已建立信任”，并立即进入认证连接步骤；短码只是会合手段，不是长期连接凭据。
4. 系统按局域网、已配置的信令和中继顺序尝试建立认证连接，分别显示正在连接、需要网络设置、目标离线、版本不兼容或已连接，不用统一的“连接失败”。
5. 连接成立后先同步签名 Agent 目录，再同步双方各自的设备库存；两个落库屏障分别显示进度，不能以 DataChannel 已打开代替数据可用。
6. 新设备为本地槽位计算 Mesh 范围的账号关联键。能可靠命中的槽位加入已有 AccountBinding；无法命中的槽位进入一次性三选确认清单：“已有登录的新位置 / 已有 Agent 的另一账号 / 全新 Agent”。
7. 只有成员信任、当前认证连接、目录落库和首份库存落库都成功，向导才显示“可以使用”；完成后庭院只增加真正的新 Agent，不因增加设备复制已有的猫。
8. 若已配对但当前无路由，设备仍保留为“已信任 · 尚未连接”，并给出设置网络、让对方上线或重新尝试等确切下一步；不得要求用户重新配对。

短码只是会合手段，不是唯一认证秘密。邀请码必须高熵、短时有效、单次使用并限速。

现有“接收连接 30 分钟”只保留在设备中心的高级恢复/局域网诊断流程，用于双方已建立信任但自动路由不可用的场景；普通添加设备向导不把它作为必经步骤，也不让用户把临时端口开放误解为长期在线。
...
1. 用户聚焦一条来源明确的本机会话，在右下动作坞选择次级动作“交接任务”。
2. Main 重新扫描来源会话，界面明确显示当前能生成“原生会话”还是“只读会话内容”。
3. 用户填写本次目标、已完成、下一步、阻塞/风险和验收标准，并决定是否携带 Git 检查点与附件。
4. 用户选择交付方式：目标是同一 Personal Mesh 且双方协商 `task.package.transfer.v1` 时，可以选择直接发送到明确设备；其他情况通过系统保存框生成便携 `.agentdesk-task`。附件始终只能通过系统选择器加入。
5. 系统固定会话和项目现场、逐项哈希并生成同一种加密包。便携模式只显示一次解锁码，文件与解锁码分开发送；直接模式不把解锁码交给 Renderer，而由 Main 将它封装进绑定目标设备与 transferId 的一次性加密 envelope。
6. 直接接收方在“活动”弹窗看到来源设备、来源 Agent、包大小和检查点摘要，在 `task.package.receive` 已授权的前提下逐次明确接受；便携接收方选择文件并输入解锁码。两种路径随后执行完全相同的解密、清单校验和逐项哈希。
7. 校验通过后，接收方选择本机 Agent 与运行位置以及任务资料保存目录。
...
6. 应用退出、设备重启或网络中断后 Job 从最后完成步骤继续；后台恢复和轮询只观察、准备并提交状态，不打开下载页、登录页或官方客户端。需要外部界面的步骤停在明确等待态，直到用户再次点击；重复点击不重复造 Slot，后台检查也不能吞掉后到的显式点击。
...
聚焦单条本机会话时，“交接任务”作为同一动作坞中的次级动作，位于“打开当前位置”和现有导出动作之间；它不能成为填充主按钮，也不进入批量选择状态。TaskPackage 的接收入口和本地历史放在“活动”弹窗的固定 Command Bar/Content 中。导出与导入各使用统一的固定 Header、可滚动 Content 和固定事务 Footer，顶部关闭与底部取消/提交在内容滚动时始终可见；这两个子弹窗不新增主页面区域。
...
“添加设备”打开受控任务子弹窗，而不是把邀请码、连接按钮、网络设置和库存结果同时堆在设备详情中。子弹窗固定 Header 与 Footer，Content 只展示当前一步，并在顶部用短进度表达“确认身份 → 建立信任 → 连接设备 → 同步目录与库存 → 可以使用”。已经完成的步骤只显示结果摘要；失败停在所属步骤并给出一个主要修复动作。关闭向导不撤销已经签发的成员关系，重新打开时从持久事实恢复进度。已配对但未连接的设备详情主操作是“继续连接”，不是再次“添加设备”。

邀请码默认只在建议信任的第一步出现；“接收连接 30 分钟”、手工地址和网络自检收进“高级恢复”，并明确临时开放状态与剩余时间。完成页直接提供“查看这台设备的 Agent”“查看全部会话”或“发送任务包”等下一步，但这些导航仍遵守第 11.11 节的原子上下文迁移，不在向导内部复制工作台。
...
### 12.5 TaskPackage 与前三种传输语义的关系

TaskPackage 是用户显式创建的便携交付物，不是 Inventory、SessionPointer 或 SelectedFiles 的组合开关：

| 能力 | 持续性 | 主要用途 | 是否带会话正文 | 是否改变来源 |
|---|---|---|---|---|
| Inventory | 来源设备持续发布索引 | 搜索和选择 | 否 | 否 |
| SessionPointer | 一次发送位置引用 | 在同一 Mesh 找到来源 | 否 | 否 |
| SelectedFiles | 一次发送明确文件 | 补充有限资料 | 仅用户所选文件 | 否 |
| TaskPackage | 一次不可变工作快照 | Agent、设备或人与人之间接手整项工作 | 原生会话或只读 transcript | 否；导入生成副本/资料 |

TaskPackage 可以先通过普通文件分享完成跨人交接，也可以在以后作为受认证文件传输的一个已知 MIME/扩展名载荷。无论通过哪条通道，包内清单、内容哈希、加密、目标适配器验证和来源保留规则都保持一致。

同一 Personal Mesh 内的直接传送采用独立的 `task.package.transfer.v1` 协议 feature 与 `task.package.receive` 能力。发送端仍先生成标准、不可变的加密 TaskPackage 字节流，再复用 transfer 通道的分块、背压、续传和整包校验；不能把会话正文、Git patch 或附件拆成另一套远端消息。一次性解锁码由 Main 生成后立即封装为绑定 `meshId + sourceDeviceId + targetDeviceId + transferId + packageHash` 的目标设备加密 envelope，目标 Main 只在用户接受且密文包完整后短时解封到受控导入事务，成功、拒绝、取消、过期或失败都清除内存与 staging。Renderer、传输状态、`task-package-history.json`、日志和普通数据库只接触 transferId、展示摘要和状态，不接触明文解锁码。

协议不兼容、能力关闭、目标离线超出当前队列策略或接收方拒绝时，来源包保持可重新导出；UI 提供“保存为便携加密文件”，不自动改走普通 `file.receive`，也不降低为未加密载荷。
...
当前兼容基线（2026-08-13）：危险权限 capability 与线协议 feature 明确分开。`connection.hello/ready` 的设备签名 payload 只接受有界精确白名单，并协商 `catalog.events.v1`、`catalog.snapshot.v1` 与 `inventory.device-facts.v1`；未知、重复、超长或畸形 feature 被忽略且不写入设备目录。新端双方优先交换目录事件向量并只发送对方缺少的原始签名事件；只支持 0.9.4 基线的旧端继续使用完整目录快照，完全没有声明目录 feature 的更旧端进入 inventory-only 兼容路径。任一侧当前没有 `catalog.manage` 时发送有界 `connection.catalog-unavailable`，库存只读能力继续可用，不再因单边权限不对称等待超时。握手同时携带当前运行时版本、系统和协议元数据并刷新设备记录，避免设备卡永久停留在配对时的旧版本；feature 和运行时元数据本身都不授予权限。
...
当前目录基线（2026-08-13）：schema v6 已实现 `catalog.events.v1`。每个目录变更保存原始来源设备签名、来源内连续序号、Lamport 逻辑时钟、因果父事件、来源操作基线和字段级操作；连接握手交换每个来源已连续取得的序号，只补发缺少事件。接收端逐条验证 Mesh、成员证书链、`catalog.manage` 角色、设备签名、来源序号冲突和撤销生效时间，再从完整已验证事件集确定性物化 Agent、AccountBinding、Blueprint 与 tombstone；转发设备不能改写原始签名。单个签名事件限制为 144 KiB，每批目录消息限制为 160 KiB，低于物理双机已经验证的 192 KiB 完整 DataChannel 信封预算；完整事件日志有 8,000 条当前安全上限。达到长期保留上限前仍需实现带签名 checkpoint 的事件压缩，当前不能静默丢弃历史或 tombstone。

`catalog.snapshot.v1` 继续作为 0.9.4 旧端兼容路径：旧快照先与本地 tombstone 安全合并，再由接收设备转换为一条带 `originDeviceId` 的签名兼容事件，因此旧端仍可参与升级过渡，但不能把已删除对象写回来。没有声明任何目录 feature 的更旧端仍只走 inventory-only；现代 inventory 不携带或裁剪全局目录。双方完成目录事件或兼容快照落库后，首份来源设备 inventory 才能满足连接完成屏障。

#### 17.1.1 双端编辑时的权威判定

全局 Agent 的权威不落在某台电脑，也不落在最后到达的一份快照。它由“已验证事件集合 + 固定物化规则”共同构成：

1. 一台设备看到另一台设备已经提交的事件后再修改，新的事件把当前因果头作为父事件，因此明确覆盖自己已经看见的旧值。
2. 两台离线设备同时修改不同普通字段，例如一边改名称、另一边改分组，字段分别保留并自动合并。
3. 两台离线设备同时修改同一普通字段时，按 `(Lamport clock, sourceDeviceId, eventId)` 的固定顺序选择同一个结果，另一项进入本地审计。任意设备随后基于完整事件头再次修改，都会成为明确的新版本，而不是永久锁死在机器 ID 的平局结果上。
4. 合并、拆分和删除属于关系事务。提交前必须持有当前 catalogRevision 且没有已知因果缺口；两个离线关系事务后来发现触及同一对象时不拼接半份结果，删除事务优先，其他冲突确定性拒绝其中一项并留审计，用户可在收敛后的新基线上重新操作。
5. 删除是单调事实：同一 Agent 或 AccountBinding 的 tombstone 一旦进入已验证事件集，任何更旧或并发的 upsert、旧目录快照和设备 inventory 都只能被抑制。主动重建使用新 ID，不能复用旧 ID 绕过删除。
6. Device 事实采用另一套权威边界：设备 A 只写设备 A 的 Slot、Deployment、SessionReplica 与本地路径。目录可以把引用已删除对象的 Slot 变成 suppressed，但不能删除 Profile、官方客户端数据或项目文件。

这套规则让任意可信设备都能编辑全局员工库，同时避免把某台机器升级成永久主机。普通字段自动收敛；会改变对象关系的操作在历史不完整时停止；删除不会被离线库存复活。
...
当前实现说明（2026-08-13）：`mesh.db` 已升级到 schema v6。v4 继续以可空 `agent_slots.agent_id/account_binding_id` 准确持久化 suppressed Slot；v5 新增 `agent_blueprints`、`agent_deployments` 与 `provisioning_jobs`；v6 把此前预留但未使用的 `catalog_events` 固化为无 Device 外键的原始签名事件日志，并独立保存 `source_sequence` 与 `lamport`。事件不能因来源设备稍后被移除而级联消失，撤销只拒绝撤销生效后的新事件；此前签署的合法历史仍可由其他可信设备补齐。现有数据库升级前使用参数化 `VACUUM INTO` 生成包含已提交 WAL 的一致快照，fsync、校验旧 schema 版本、`integrity_check` 与外键后，再以同目录原子无覆盖方式发布 `pre-v6` 回滚点并启动事务迁移；Node 22 测试运行时与 Electron 43 内嵌 Node 均走同一路径。
...
建议新增的 Renderer 白名单方法：
    ...
    taskPackages:previewExport
    taskPackages:export
    taskPackages:chooseImport
    taskPackages:inspectImport
    taskPackages:commitImport
    taskPackages:cancelImport
...
| 恶意或伪造 TaskPackage | 外层版本/KDF 白名单、AES-GCM 认证、清单/逻辑路径/数量/单项与总量上限、逐项 SHA-256、目标适配器身份复核、冲突时拒绝覆盖 |
| TaskPackage 解锁码泄漏 | 高熵一次性码只显示一次，不写入包/历史/日志，提示与文件分开传输；解密明文只进私有临时目录并按生命周期清理 |
| 同 Mesh TaskPackage 直送泄漏或错投 | 独立接收能力与逐次确认；解锁码只进绑定 Mesh、来源、目标、transferId 与包哈希的目标设备加密 envelope；Renderer、历史、日志和持久化存储不接触明文 |
...
| 设备被撤销 | 立即断开、清缓存、拒绝重连 |
| Profile 的 Crashpad handler 陷入重复自崩溃 | 100 文件或 200 MiB 硬上限持续生效；一分钟内 5 个同尺寸 dump 触发持久熔断。当前 owned Profile 被关闭；AgentDesk 自己的受管 Profiles 根还会按精确 `user-data-dir` 关闭旧版本或异常退出遗留的孤儿进程；无所有权的官方默认/custom 目录不被接管 |
...
- AgentDesk 所启动的 Profile 默认随管理器正常退出而关闭；同一 `user-data-dir` 不重复启动，外部启动的同路径进程在 AgentDesk 解除所有权后不能被普通关闭误杀。只有崩溃风暴或容量无法安全收敛这一磁盘事故路径可以停止无当前所有权记录的遗留进程，并且该例外只适用于 AgentDesk 自己的受管 Profiles 根；官方默认/custom 目录仍失败关闭。
...
- 签名目录事件的证书链、原始设备签名、来源连续序列、Lamport 时钟、因果父事件、缺口向量、有界批次与转发后原签名保持；
- 双端并发修改不同字段自动合并，同字段按固定顺序收敛并只记录一次冲突；并发拆分不能越过删除，删除后同 ID 不复活，主动重建使用新 ID；
- 签名握手 feature 只接受精确有界白名单；新旧双向兼容、目录权限不对称不会发送未知消息或拖垮 inventory.read；认证握手刷新当前 appVersion 而不是永久使用配对时版本；
- `inventory.device-facts.v1` 不携带全局 Agent/Binding/tombstone，旧兼容投影只能在当前 `catalog.manage` 下增补且不能覆盖、裁剪或复活目录对象；
- schema v3/v4/v5 → v6 迁移先保留可恢复备份，保留 Slot 数据与外键并建立 Blueprint/Deployment/Job/签名目录事件表；事件历史不对来源 Device 建级联外键，三种目录删除写入 nullable suppressed Slot，关闭重开后仍可删到零；
...
- 输入状态释放；
- 文件清单和路径越界；
- TaskPackage 错误解锁码、密文篡改、清单类型伪装、重复逻辑路径、单项/总量上限和逐项哈希；
- Codex TaskPackage 根会话与 internal-child 完整捕获、目标身份复核、同 ID 不同内容冲突、幂等导入、导入标题来源标注和失败回滚；
- Git 检查点只携带已跟踪差异，未跟踪内容仅在用户明确选择为附件时进入包；打开客户端和历史记账失败不能删除已经提交的会话或资料；
- 首次向导状态按版本保存，未实际展示完成不得提前标记；创建首个 Agent 的本机目录/设备身份事务幂等，失败或重开不重复造 Agent、设备或 Slot，也不产生网络监听；
- ProvisioningJob 普通启动恢复、密钥解锁恢复和定时轮询不得打开安装页、登录页或官方客户端；已有身份只在后台提交 ready，显式点击在并发后台检查期间也必须随后得到执行；
...
- 离线队列；
- 分页库存同步；
- 全新安装从创建第一个 Agent 到本机首次准备；已有 Profile 的迁移预览、取消和恢复；整个流程不产生端口监听或公网租约；
- 两个 endpoint 从邀请、双方身份确认、成员信任、认证连接、目录落库到库存落库完成设备向导；分别注入无路由、离线、版本不兼容和库存失败，验证准确恢复步骤；
- 同 Mesh TaskPackage 直接发送、接收拒绝/接受、传输中断续传、密钥 envelope 篡改、导入回滚和旧端便携回退；
- 多设备同时在线；
- 同一目标只允许一个输入 owner；
- 版本降级；
- 设备撤销；
...
### 26.2.1 v4/v5 到永久员工库与事件目录的迁移

- `mesh.db` 升级到 schema v6 前先保留可恢复备份；`profiles.json` 和官方客户端目录不移动；
...
- v5 预留的空 `catalog_events` 表迁移为带来源序号和 Lamport 时钟的 v6 事件表；既有 Agent、Binding、Blueprint 与 tombstone 由本机设备签署 bootstrap 事件，设备移除不级联删除已验证历史；
- 0.9.4 旧客户端继续通过 `catalog.snapshot.v1` 兼容；未声明 `catalog.events.v1` 或 `catalog.snapshot.v1` 的更旧客户端只走 inventory-only 兼容投影。新端不向旧端发送未知目录消息，接收端也只在当前 `catalog.manage` 下允许旧 inventory 增补目录对象，不能接受删除、覆盖或使无 Slot 员工消失的写入；
...
当前状态：**本机、加密配对与签名目录事件纵向基础已实现，Phase 2 继续进行**。顶栏已有“设备”入口和本机设备中心；用户可显式建立本机设备身份，把现有 Identity Group 迁移为 AgentIdentity/AccountBinding/AgentSlot，查看目录统计、重命名本机、离开并重置 Mesh。设备中心已经可以生成一次性配对码、加入已有 Mesh、逐项调整远端权限以及撤销删除设备；局域网端口只在用户创建邀请后临时开启。同一强账号标识在两个隔离端点的自动归并已通过测试。schema v6 已修复 suppressed Slot 持久化并落地无永久主机的签名事件目录：普通字段并发收敛，关系事务按基线和因果缺口门禁，删除 tombstone 防复活。物理双机 0.9.5 双端并发编辑仍需用户升级另一台设备后复验，不绕过 Phase 1 的其余真机退出条件。
...
- 无目录的新用户可从“创建第一个 Agent”原子建立本机目录与设备身份，但不开放任何网络；已有 Profile 先预览无损迁移。
- 添加设备向导把成员信任、当前连接、目录同步和库存同步分别呈现并可恢复；“接收连接 30 分钟”只作为高级恢复。
...
- 普通启动、后台恢复与轮询不会自动拉起安装、登录或客户端界面；显式“继续登录/打开账号”仍能在后台检查完成后准确执行。
...
- Codex 原生捕获/导入适配器与来源标题；
- 活动弹窗中的导入和本地历史；
- 跨 Agent、跨设备和跨人的同一安全交接语义。
- 同一 Mesh 内以 `task.package.transfer.v1` 复用分块传输，并以独立 `task.package.receive`、逐次接受和目标绑定密钥 envelope 保护直送。

退出条件：

- 错误密钥、篡改、越界大小、重复路径、适配器不兼容和同会话 ID 内容冲突均在写入前停止；
- 原生导入失败回滚，成功后打开客户端失败不删除已经导入的内容；
- 未跟踪文件正文、项目全目录、凭据和本机环境不被自动打包；
- 来源保留，标题明确标出交接来源，接收端可以从任务说明、会话和代码检查点继续；
- “复制会话信息”仍为唯一填充主按钮和唯一路径/坐标文本契约；
- 同 Mesh 直接发送在协议协商、能力授权、接受确认、断点恢复、拒绝/取消清理、旧端便携回退和双端物理验收完成前不宣称完成；
- 跨 Mesh 身份确认和非 Codex 原生导入在实际实现前不宣称完成。
...
33. 两台离线设备分别修改同一 Agent 的不同普通字段后自动合并；修改同一字段后稳定得到同一值并保留一次冲突审计；随后任意设备的新编辑可以正常覆盖该结果。
34. 任一设备显式删除 Agent 后，另一台设备的旧 Slot、旧目录快照、并发改名或后续旧 inventory 都不能恢复同一 agentId；同名重建必须产生新 agentId。
35. 设备卡中的 appVersion 在下一次认证握手后反映当前运行版本，不永久停留在首次配对版本。
36. 用户可以从一条本地会话显式生成 TaskPackage，准确看见它将携带原生会话还是只读内容，并自行填写目标、进展、下一步、风险和验收。
37. TaskPackage 可以交给另一个本机 Agent、另一台设备或另一个人；接收方验证后再选择目标 Agent/Profile，来源副本始终保留。
38. Codex 原生 TaskPackage 导入后，目标客户端能扫描到同一根会话与内部记录，标题标注来源；同 ID 不同内容绝不覆盖。
39. 解锁码不进入任务包、本地历史或日志；任务包文件不明文暴露标题、Agent、项目和文件名，解密草稿按生命周期清理。
40. TaskPackage 不改变 Personal Mesh 的目录权威、不自动建立团队成员关系、不自动同步项目或合并来源与接收端后续变化。
41. 全新安装可以直接创建第一个 Agent；该动作只原子建立本机目录与设备身份并进入首次准备，不开放端口、发布租约或连接远端。已有 Profile 时必须先预览无损迁移，首次向导的展示完成状态按版本准确持久化。
...
46. AgentDesk 启动同一 Profile 前必须识别既有 `user-data-dir` 实例；其正常退出默认关闭所启动的 Profile，Crashpad 风暴不得无限写盘。升级、管理器强制退出或重启后，AgentDesk 自己的受管 Profiles 根内精确匹配的孤儿进程若继续制造已确认的崩溃风暴，也必须被事故熔断停止；普通用户关闭仍不得越过进程所有权，无所有权的官方默认/custom 目录不得被事故路径误接管。持续一小时的故障注入中，每个 Profile 的 `Crashpad/pending` 不得超过 100 个文件或 200 MiB；安全清理后会话、归档、配置、SQLite、`codex-home` 与留存诊断样本保持完整，多 Profile 的限额和熔断互不串扰。
47. 普通启动、后台恢复、密钥解锁恢复和轮询只观察 ProvisioningJob；没有该 Profile 的身份信息时保持等待登录，已有身份时可提交 ready，但两种情况都不得自动打开下载页、登录页或官方客户端。外部界面只能来自明确用户动作或已经完成的远端有人确认。
...
- 把“发送会话信息”做成新的交接模板系统；
- 把一次 TaskPackage 快照误写成持续任务队列、自动编排或双向同步；
- 为了“移动”体验在接收成功后自动删除来源，导致不可恢复的数据丢失；
...
- 多显示器坐标和 IME；
- 原始会话格式不稳定；
- 原生 TaskPackage 适配器追不上客户端格式变化，或把部分会话记录误报为完整迁移；
- 大型 TaskPackage 在解密、逐项校验或磁盘不足时留下明文暂存；
...
- 官方客户端或 Crashpad handler 自崩溃后被持续重拉，形成孤儿进程并快速写满磁盘；仅依赖客户端自己的崩溃报告策略不足以保护 AgentDesk 管理的数据卷。
...
- 将 ProvisioningJob 的后台恢复与交互推进彻底分开：应用启动、密钥解锁后的恢复和定时轮询只检查客户端、准备受管目录、观察身份并提交状态，不再因首次缺少 `login-started` 自动拉起 Codex、Claude 等官方客户端，也不会打开安装或登录网页。
- 后台识别到该确切 Profile 已有账号身份时可幂等提交 Profile/Slot/Deployment 为 ready，但不会再次启动客户端或记录为已打开；用户明确点击“继续登录/打开账号”时才允许外部界面。
- 修复显式点击与后台检查并发时被同一 in-flight Promise 吞掉的问题：后台观察先完成，随后重放一次显式交互推进；Codex/Claude 共用的 ProvisioningService 回归覆盖无身份、已有身份、重启恢复和后续主动打开。
...
- 建立 Profile 本机资源监管：AgentDesk 记录自己启动的受支持官方桌面客户端，启动前精确检查同一 `user-data-dir`，阻止重复实例；进程自然退出后释放所有权，不能把后来由用户启动的同路径进程误当成受管进程。
- 默认退出策略改为先关闭 AgentDesk 所启动的 Profile 及其同目录 Crashpad 进程并复核退出；收口失败时取消 AgentDesk 退出。设置中保留显式“保留客户端运行”，同时明确管理器退出后不再监控；强制结束 AgentDesk 或操作系统崩溃不冒充正常退出保证，下一次启动会根据持久记录和精确 Profile 路径恢复监管。
...
- 补齐升级与异常退出后的孤儿进程边界：普通关闭和正常退出仍严格要求当前 AgentDesk 所有权；只有已确认的 5 次同尺寸 dump 风暴或 Crashpad 容量无法安全收敛时，监管器才可对 AgentDesk 自己的受管 Profiles 根按精确 `user-data-dir` 停止匹配进程。无当前所有权的官方默认/custom 目录不进入后台扫描、删除或停机；成功停止后立即清除持久所有权与 launch PID，防止后来用户启动的同路径进程被普通关闭误认。
...
- 新增成品首次使用 smoke：只接受 `AgentDesk.app`、`win-unpacked` 或带版本 portable，以临时 userData 完成首次初始化、重启恢复并完成、完成后再重启；随机 launch token 与 Browser command line 绑定本次启动，只向原始 spawn 句柄发终止信号，验证固定窗口骨架、唯一 Agent/本机设备、零默认 Profile、零远端设备、零 Mesh 连接及进程/回环调试端点清理。`main` 的 ad-hoc macOS 兼容任务使用独立语义开关：runner 显式以 `codesign --verify --all-architectures` 验证整体，从主程序枚举所有架构并逐架构读取签名，要求每个切片都恰为无 Authority、无 TeamIdentifier 的 ad-hoc 签名，再为三次启动加入 Chromium 原生 mock-Keychain 参数并从 Browser command line 逐次核对；Windows、未签名包、混合签名和 Developer ID/其他证书签名包均失败关闭。
- `main` 的 macOS CI 构建 ad-hoc universal unpacked 包，核对主程序/helper 双架构并以受限 mock Keychain 执行 fuse/ASAR 与首次使用事务；该结果不证明 macOS 系统 Keychain/OS 密钥保护。签名发布、Draft 重下载和公开匿名重下载不允许传入语义或原生 mock-Keychain 开关，继续使用系统 Keychain。Windows CI 先核对无签名 `win-unpacked`，再解开 portable 并复验其真实内层主程序/ASAR/helper，随后分别执行 smoke。两端 `main` 任务都是打包兼容证据，不是可分发的签名产物。
...
- 当前完整 Node 套件 517 项中 516 通过、1 项仅 Windows 跳过、0 失败；真实 Electron UI 21/21，发布安全定向 14/14。当前本机 thin arm64 macOS unpacked 包已通过 fuse/ASAR verifier，并在逐架构门禁确认唯一 arm64 切片为 ad-hoc 后，以真实语义开关通过三次 mock-Keychain 首用；这不是 universal CI、系统 Keychain 或签名发布证据。
...
- 所有者批准产品与文档成熟化实施规划，新增普通首次使用主路径：“创建第一个 Agent”可以原子建立本机 Agent 目录与设备身份并继续首次准备，但不得开放端口、发布租约、连接远端或授予远端能力；已有 Profile 必须先预览无损迁移；
- 冻结设备任务向导：身份确认、成员信任、认证连接、目录落库与库存落库分别表达、分别恢复，配对成功不再冒充已经可用；“接收连接 30 分钟”、手工地址和网络自检降为高级恢复；
- 批准同一 Mesh 内直接发送标准 TaskPackage：新增 `task.package.receive` 与 `task.package.transfer.v1`，继续复用不可变包、分块传输、逐项校验和来源保留；接收端逐次确认，旧端明确退回便携加密文件；
- 冻结直送解锁码边界：只进入绑定 Mesh、来源/目标设备、transferId 和包哈希的目标设备加密 envelope，明文不得进入 Renderer、包、历史、日志或持久化存储；
- 冻结成熟化发布门禁：当前 0.10.0 不补发为稳定版；真实公网 NAT/TURN、长连接恢复和 macOS/Windows 权限矩阵未关闭时只发布签名、公证且明确标记的 Preview，建议首个候选为 `v0.10.1-preview.1`。

### 1.25 — 2026-08-14

- 新增 TaskPackage 领域对象，冻结“WorkPackage 是持续责任节点、TaskPackage 是某一时刻的不可变交付快照”；它与 Inventory、SessionPointer、SelectedFiles、Agent catalog 和设备事实分别建模；
- 冻结跨 Agent、跨设备与跨人的整项工作交接：人工填写目标/进展/下一步/风险/验收，携带原生会话或只读 transcript、Git remote/branch/head/已跟踪差异和明确附件；来源始终保留，不自动同步、合并或删除；
- 落地便携 `.agentdesk-task`：流式 AES-256-GCM、scrypt 一次性高熵码、外层元数据隐藏、清单/逻辑路径/单项和总量上限、逐项 SHA-256、私有解密 staging 与本地无密钥历史；
- 落地 Codex 原生适配器：一致快照根 JSONL 与归属 internal-child，目标端重新验证身份、拒绝同 ID 异内容覆盖、事务回滚并将标题标注为来自交接人/来源 Agent；其他已有 Markdown 导出能力的客户端先使用只读内容；
- 右下单会话动作坞增加非主按钮“交接任务”，活动弹窗增加导入与历史；导出/导入使用固定 Header、滚动 Content 和固定事务 Footer，保持一个 Header、一个 Footer、三个面板和“复制会话信息”唯一主操作不变；
- 当前跨人/跨 Mesh 交接依赖用户自行传递加密文件与分开发送的解锁码；同 Mesh 直接发送、跨 Mesh 身份确认和更多原生适配器继续列为后续，不把便携文件误报成团队平台或 P2P 已交付；开发包版本升为 0.10.0。
- 收紧不可变与接收边界：目标成为 schema 必填项，附件先固定到私有 staging 并拒绝复制中变化，Git 只接受前后复读一致的 HEAD/状态/二进制差异，Codex JSONL 限制单行大小并复核首条 `session_meta`，清单拒绝未被会话或项目引用的伪装条目，同一来源 Profile 不作为自己的导入目标；
- 当前完整 Node 套件 436 项中 435 通过、1 项仅 Windows 跳过、0 失败；真实 1040 × 840 Electron 以 18/18 任务路径通过，新增实际 TaskPackage 导出/接收弹窗的固定 Header、单一 Content 滚动、固定 Footer、父子层与焦点回归证据。

### 1.24 — 2026-08-13

- 将全局 Agent 权威从“最后一份目录快照”升级为无永久主机的签名事件集合：事件保存来源设备签名、来源连续序号、Lamport 时钟、因果父事件、操作基线和字段级变更，握手以向量只补缺失事件并允许可信设备转发原始签名；
- 冻结双端编辑规则：因果后的编辑覆盖已见版本，并发不同字段自动合并，并发同字段确定性收敛且留审计；关系事务要求当前 revision 与完整因果历史，删除优先于冲突事务；
- 删除 tombstone 对同一对象 ID 永久生效，旧库存、旧快照和并发 upsert 均不能复活；主动重建必须使用新 ID，本地 Profile 与第三方数据只转为 suppressed，不被目录事件删除；
- `mesh.db` 升至 schema v6，目录事件独立保存 source sequence 与 Lamport，取消对 Device 的级联外键，使来源设备移除后合法历史仍可补齐；撤销只拒绝生效后的新事件；
- 保留 0.9.4 `catalog.snapshot.v1` 兼容，旧快照经 tombstone 合并后转换成本机签名兼容事件；认证握手新增当前运行时元数据，设备版本在重连后刷新；开发包版本升为 0.9.5；
- 把完整签名事件和目录批次分别压到 144/160 KiB，保持低于物理双机已验证的 192 KiB 最终 DataChannel 信封预算；协议严格拒绝未定义的顶层、操作和对象字段，已有 Blueprint 后续变化也会补入事件流；
- 自动化新增双端真实 MeshService 并发字段编辑、同字段冲突、删除对并发旧改名、同名新 ID 重建、事件签名/转发/缺口向量、有界批次与 v3/v4 → v6 一致迁移回归。物理双机 0.9.5 双端编辑、事件 checkpoint 压缩和冲突专用 UI 仍待后续验收。
