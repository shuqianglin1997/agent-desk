...
- 技术自检使用一次性隐藏 Renderer；真实查看/控制使用限定在主窗口右下详情面板内的专用沙箱 Remote Surface WebContentsView；
...
4. 用户返回工作台时 Main 先释放输入并隐藏 Remote Surface，可按明确的后台查看状态保留媒体；Surface 被销毁、Renderer 崩溃或用户显式断开时终止连接并再次释放输入。
