...
- 设备、AgentIdentity、AccountBinding、AgentBlueprint、AgentDeployment、ProvisioningJob、AgentSlot 与 tombstone 写入独立 `mesh.db`，不污染 `profiles.json`；当前 schema v5 保留 v4 的 nullable suppressed Slot 语义，并在升级前生成包含已提交 WAL、经版本/完整性/外键校验的 `pre-v5` 回滚点；
...
- 全局 Agent/AccountBinding/Blueprint/tombstone 通过独立签名 `catalog.snapshot` 同步；握手把危险权限与线协议 feature 分离，只协商精确白名单 `catalog.snapshot.v1` / `inventory.device-facts.v1`。旧端安全降级为 inventory-only，不接收未知目录消息；目录权限不对称显式跳过目录屏障，不会拖垮 `inventory.read`；
