...
会话来自本地只读扫描，不要求手动录入。统一字段包括标题、账号、应用、创建 / 活跃时间、项目目录、来源、状态、模型、线程 ID、文件路径和稳定会话标识。列表支持当前账号 / 全部账号范围、精简 / 详细视图，以及所有属性表头的升降序排列。

交接清单可跨账号保存多个勾选项，并按用户调整的优先级合并复制。每个会话会另外索引交接资料：Claude CLI 的 `ExitPlanMode.planFilePath`、Codex 的最新 `update_plan` 作为明确关联默认勾选；项目内 `PLAN / TODO / TASKS / ROADMAP / HANDOFF` 等时间相关文本只作为候选，需用户手动确认。交接始终不复制完整聊天内容。
