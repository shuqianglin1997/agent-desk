...
当前开发版已经贯通有人值守 Personal Mesh 的代码链路：设备配对、全局 Agent 目录、跨设备会话库存、会话信息发送、选定文件传输、远程查看、键鼠控制、多设备控制台，以及局域网优先、签名信令回退、STUN/TURN 配置和连接诊断。Agent、AccountBinding、AgentSlot 的新增、归属、合并、拆分和三种删除范围已经进入真实语义 IPC；Device Lens、Agent、Slot、会话焦点/勾选、副本来源、远控会话和传输草稿使用独立 UI 上下文。自动化、单机双隔离端点的真实 Electron WebRTC，以及真实 1040 × 840 窗口的 13 条任务路径均已通过；两台物理电脑、真实公网 NAT、coturn 强制中继及 macOS/Windows 四向权限矩阵仍是公开发布前验收，不得把本机证据写成真机矩阵完成。
...
- 主 Renderer 的 Device Lens、Agent、Slot、focused/checked 会话、SessionReplica、设备详情、活动远控和两类传输草稿彼此独立；render、搜索和刷新不会自动选择第一项或静默改写动作目标。
...
- 1040 × 840 七行窗口能通过本地 13 条任务路径验收：双 Presenter/三语/明暗主题、focus 与勾选、设备中心原子导航、Agent/Binding/Slot 管理、多副本显式来源、独立传输草稿、远控后台提示、撤销清理和 reduced-motion。
