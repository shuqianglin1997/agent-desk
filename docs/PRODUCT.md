...
工作图把“任务”和“当前执行它的会话”分开建模。首版提供一个可解释、可恢复的固定拓扑：

1. 从跨账号交接选择中创建多个并行任务节点。
2. 每个节点绑定一个现有会话，拥有独立分工、状态、完成说明、Agent 输出和重试信息。
3. 只有全部上游节点完成或跳过时，`ALL` 汇合点才就绪。
4. 汇合包只包含各节点分工、结果、会话元信息和最终勾选的规划资料，不复制完整聊天；内容按字节限额截断。
5. 选择一个非上游会话作为综合节点。`review` 模式等待人工投递；`automatic` 模式在汇合后自动启动综合 Agent。

执行方式分为两种：`observe` 只观察已有会话并由用户标记完成；`managed` 在一次总确认后复用安全 runtime，为每个节点启动兼容的本地 Agent。Agent 返回 `ready` 才算节点完成；异常退出和中断进入可见的失败 / 阻塞状态。工作图单独保存在 `workgraphs.json`，有原子写入、常规备份、更新前快照和重启恢复。
...
- 工作图的自动执行默认关闭；开启后仍复用 runtime 的身份、工作区、权限、实例数和输入 / 输出限额。
