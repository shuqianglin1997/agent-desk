...
当前开发版已经贯通有人值守 Personal Mesh 的代码链路：设备配对、全局 Agent 目录、跨设备会话库存、会话信息发送、选定文件传输、远程查看、键鼠控制、多设备控制台，以及局域网优先、签名信令回退、STUN/TURN 配置和连接诊断。Agent、AccountBinding、AgentSlot 的新增、归属、合并、拆分和三种删除范围已经进入真实语义 IPC；Device Lens、Agent、Slot、会话焦点/勾选、副本来源、全局弹窗、远控会话和传输草稿使用独立 UI 上下文。主窗口已收敛为一个 Header、顶部 Agent、左下会话、右下统一详情三个固定面板和一个 Footer；设备、工具、活动、设置各自打开独立弹窗，右下只承载会话、额度和隔离远控，会话动作归右下详情底部动作坞，Footer 只显示全局状态、今日账本和提醒总开关，庭院不再插入小账本/提醒行。自动化、单机双隔离端点的真实 Electron WebRTC，以及真实 1040 × 840 窗口任务路径均已通过；两台物理电脑、真实公网 NAT、coturn 强制中继及 macOS/Windows 四向权限矩阵仍是公开发布前验收，不得把本机证据写成真机矩阵完成。
...
- 1040 × 840 窗口稳定保持 Header、Footer 与顶部 Agent/左下会话/右下详情三个面板；设备/工具/活动/设置各用独立弹窗且不改写底层工作台，会话动作只进入右下详情动作坞，Footer 只保留全局账本/提醒状态，庭院没有额外状态条。完整本地任务路径覆盖三语/明暗主题、庭院/卡片、focus 与勾选、四个 Header 弹窗、设备中心原子导航、Agent/Binding/Slot 管理、多副本显式来源、独立传输草稿、远控后台提示、撤销清理和 reduced-motion。
