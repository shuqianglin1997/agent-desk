...
这里是仓库文档的入口，也是“当前做到哪一步”的统一说明。涉及 Personal Agent Mesh 的实现决策，以 [`PERSONAL_AGENT_MESH_PLAN.md`](PERSONAL_AGENT_MESH_PLAN.md) 为唯一实施权威；当前权威版本为 **1.28 / OWNER APPROVED — IMPLEMENTATION AUTHORIZED**。
...
HTML 审阅稿记录当时的设计审阅过程；历史方案和旧图中的界面结构不能覆盖 1.28 权威与当前产品事实。开发接力摘要只能帮助定位，不替代全文重读权威规划。
...
检查会验证导航链接、权威版本、517 项 Node / 21 条实窗 / 14 条发布安全证据、发布事务关键阶段与发布口径，并阻止稳定版、旧七行 UI、默认 Cloud/Kimi，以及把代码、窗口投影、托管 runner 或物理验收混写为“全部完成”等越阶段说法重新进入当前事实文档。
